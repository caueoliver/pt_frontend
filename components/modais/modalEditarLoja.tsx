...
import { Loja } from '@/interfaces/lojaInterface'; // Verifique se o caminho da interface está correto
...
export function ModalEditarLoja({ isOpen, onClose, loja }: ModalEditarLojaProps) {
  // estados para os inputs (já preenchidos com os dados atuais da loja)
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 px-4 py-8">
      ...
      {/* container do modal */}
      <div className="bg-[#EBEBEB] w-[800] rounded-3xl p-8 mt-50 mb-50 relative flex flex-col gap-5 shadow-2xl max-h-full overflow-y-auto">
        ...
        {/* título */}
        <h2 className="text-center text-3xl font-medium text-black mb-2">Editar loja</h2>
        ...
        <input 
          ...
          className="w-full bg-white rounded-full px-6 py-3 text-black outline-none shadow-sm font-light"
        />
        ...
        <div className="relative">
          <select 
            ...
            className="w-full bg-white rounded-full px-6 py-3 text-black outline-none shadow-sm appearance-none font-light cursor-pointer"
          >
            {/* percorre o vetor para criar opções */}
            {categorias.map((cat) => (
        ...
        <div className="flex flex-col items-center gap-4 mt-4">
          <button 
            className="w-full bg-red-600 text-white py-3 rounded-full font-medium hover:bg-red-700 transition-colors shadow-md"
          ...
          <button 
            className="w-2/3 bg-[#6A38F3] text-white py-3 rounded-full font-medium hover:bg-purple-700 transition-colors shadow-md"
            onClick={() => {
